window.function = async function(api_key, vector_store_id, limit, order, after, before, filter) {
    // Validate API Key
    if (!api_key.value) {
    ...
    if (filter.value) queryParams.append("filter", filter.value);

    // API endpoint URL
    const apiUrl = `https://api.openai.com/v1/vector_stores/${vector_store_id.value}/files?${queryParams.toString()}`;
